...
`);i.galleryForm.insertAdjacentHTML("beforeend",t),w.refresh()}function E(){i.galleryForm.innerHTML=""}function p(){i.loader.classList.remove("hidden")}function u(){i.loader.classList.add("hidden")}function P(){i.loadBtn.classList.remove("hidden")}function h(){i.loadBtn.classList.add("hidden")}function $(){d.warning({title:"Caution",message:"Please type something to search!",position:"topRight"})}function q(){d.warning({title:"Caution",message:"Sorry, there are no images matching your search query. Please try again!",position:"topRight"})}function b(r){d.warning({title:"Caution",message:`Error: ${r}`,position:"topRight"})}function N(){d.info({title:"Info",message:"We're sorry, but you've reached the end of search results.",position:"topRight"})}const m={formEl:document.querySelector(".search-form"),galleryForm:document.querySelector(".gallery"),loadBtn:document.querySelector(".more")};m.formEl.addEventListener("submit",O);m.loadBtn.addEventListener("click",C);let n,f,c;async function O(r){if(r.preventDefault(),c=r.target.elements.text.value.trim(),n=1,!c){$();return}h(),p(),E();try{const t=await g(c,n);if(t.hits.length===0){q(),u();return}f=Math.ceil(t.totalHits/15),y(t),n<f&&P()}catch(t){b(t)}u(),r.target.reset()}async function C(){n+=1,p();try{const r=await g(c,n);y(r);const t=m.galleryForm.firstElementChild.getBoundingClientRect().height;scrollBy({behavior:"smooth",top:t*2}),n>=f&&(h(),N())}catch(r){b(r)}u()}
